#include "../unit_tests.h"

START_TEST(s21_negate_test_1) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = 0;
  src1.bits[0] = 0b00000000000000000000000000000000;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b00000000000000000000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000000000;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b10000000000000000000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_2) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = 50/10;
  src1.bits[0] = 0b00000000000000000000000000110010;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b00000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000110010;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b10000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_3) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = -50/10;
  src1.bits[0] = 0b00000000000000000000000000110010;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b10000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000110010;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b00000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_4) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = -51/10;
  src1.bits[0] = 0b00000000000000000000000000110011;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b10000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000110011;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b00000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_5) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = -59/10;
  src1.bits[0] = 0b00000000000000000000000000111011;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b10000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000111011;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b00000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_6) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = 59/10;
  src1.bits[0] = 0b00000000000000000000000000111011;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b00000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000111011;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b10000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_7) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = 51/10;
  src1.bits[0] = 0b00000000000000000000000000110011;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b00000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000110011;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b10000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_8) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = 89741513654898/10^10;
  // 8974
  src1.bits[0] = 0b10010011000010110000001001110010;
  src1.bits[1] = 0b00000000000000000101000110011110;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b00000000000010100000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b10010011000010110000001001110010;
  origin.bits[1] = 0b00000000000000000101000110011110;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b10000000000010100000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_9) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = -89741513654898/10^10;
  // 8974
  src1.bits[0] = 0b10010011000010110000001001110010;
  src1.bits[1] = 0b00000000000000000101000110011110;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b10000000000010100000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b10010011000010110000001001110010;
  origin.bits[1] = 0b00000000000000000101000110011110;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b00000000000010100000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_10) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = -1,5;
  // 2
  src1.bits[0] = 0b00000000000000000000000000001111;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b10000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000001111;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b00000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_11) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = -2,5;
  // 3
  src1.bits[0] = 0b00000000000000000000000000011001;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b10000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000011001;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b00000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_12) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = 2,5;
  // 3
  src1.bits[0] = 0b00000000000000000000000000011001;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b00000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000011001;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b10000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_13) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  // src1 = 1,5;
  // 2
  src1.bits[0] = 0b00000000000000000000000000001111;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b00000000000000010000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000001111;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b10000000000000010000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

START_TEST(s21_negate_test_14) {
  s21_decimal src1, origin, result;
  int value_type_result, value_type_origin;
  src1.bits[0] = 0b00000000000000000000000000000000;
  src1.bits[1] = 0b00000000000000000000000000000000;
  src1.bits[2] = 0b00000000000000000000000000000000;
  src1.bits[3] = 0b10000000000000000000000000000000;
  value_type_result = s21_negate(src1, &result);
  value_type_origin = 0;
  origin.bits[0] = 0b00000000000000000000000000000000;
  origin.bits[1] = 0b00000000000000000000000000000000;
  origin.bits[2] = 0b00000000000000000000000000000000;
  origin.bits[3] = 0b00000000000000000000000000000000;
  ck_assert_int_eq(origin.bits[3], result.bits[3]);
  ck_assert_int_eq(origin.bits[2], result.bits[2]);
  ck_assert_int_eq(origin.bits[1], result.bits[1]);
  ck_assert_int_eq(origin.bits[0], result.bits[0]);
  ck_assert_int_eq(value_type_result, value_type_origin);
}
END_TEST

Suite *test_negate(void) {
  Suite *s = suite_create("negate");
  TCase *tc = tcase_create("negate");

  tcase_add_test(tc, s21_negate_test_1);
  tcase_add_test(tc, s21_negate_test_2);
  tcase_add_test(tc, s21_negate_test_3);
  tcase_add_test(tc, s21_negate_test_4);
  tcase_add_test(tc, s21_negate_test_5);
  tcase_add_test(tc, s21_negate_test_6);
  tcase_add_test(tc, s21_negate_test_7);
  tcase_add_test(tc, s21_negate_test_8);
  tcase_add_test(tc, s21_negate_test_9);
  tcase_add_test(tc, s21_negate_test_10);
  tcase_add_test(tc, s21_negate_test_11);
  tcase_add_test(tc, s21_negate_test_12);
  tcase_add_test(tc, s21_negate_test_13);
  tcase_add_test(tc, s21_negate_test_14);

  suite_add_tcase(s, tc);
  return s;
}
